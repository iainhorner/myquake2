	...
	{	// save the map just exited
		if (!strchr (map, '.') && !strchr (map, '$'))
		{
			...
			if (!CM_MapWillLoad (expanded))
			{
				...
				SV_BroadcastPrintf(PRINT_HIGH, "Can't find map '%s'\n", map);
				SV_BroadcastPrintf(PRINT_HIGH, "Type 'maplist' to see popular maps or ");
				char* noGameName = "";
				if (strcmp(game_name->string, noGameName) == 0)
					SV_BroadcastPrintf(PRINT_HIGH, "see full maplist @ http://dhg-downloads.website/q2/baseq2/maps/\n");
				else
					SV_BroadcastPrintf(PRINT_HIGH, "see full maplist @ http://dhg-downloads.website/q2/%s/maps/\n", game_name->string);
				return;
			}
		}		
