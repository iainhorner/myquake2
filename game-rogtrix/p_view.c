...
void P_DamageFeedback (edict_t *player)
{
	...
	if ((level.time > player->pain_debounce_time) && (client->invincible_framenum <= level.framenum))
	{
		r = 1 + (rand()&1);
...
//===================================
	...
	// if dead, fix the angle and don't add any kick
        if (ent->deadflag /*&& (xyspeed)*/)
        {
                //VectorClear (angles);
		if (xyspeed) {
                ent->client->ps.viewangles[ROLL] = 40*sin(level.time);
...
void G_SetClientSound (edict_t *ent)
{
	...
	else if (Q_strcmpz(weap, "weapon_railgun") == 0)
		ent->s.sound = gi.soundindex("weapons/rg_hum.wav");
	//else if (strcmp(weap, "weapon_bfg") == 0)
	//else if (Q_strcmpz(weap, "weapon_bfg") == 0)
	//	ent->s.sound = gi.soundindex("weapons/bfg_hum.wav");
	// RAFAEL
	//else if (strcmp (weap, "weapon_phalanx") == 0)
	else if (Q_strcmpz (weap, "weapon_phalanx") == 0)
