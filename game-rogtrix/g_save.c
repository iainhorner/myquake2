...
/* Nick	{"stand", FOFS(monsterinfo.stand), F_FUNCTION, FFL_NOSPAWN},
...
void InitGame (void)
{
	...
	sv_maxvelocity = gi.cvar ("sv_maxvelocity", "5000", 0);
	...
	//coop = gi.cvar ("coop", "0", CVAR_LATCH); Nick 18/09/2005 - not in DM
	//skill = gi.cvar ("skill", "1", CVAR_LATCH); Nick - 18/09/2005 - No skill in DM
	maxentities = gi.cvar ("maxentities", "1024", CVAR_LATCH);
	mapcycle_always = gi.cvar("mapcycle_always", "0", CVAR_LATCH);
...
void ReadLevel (char *filename)
{
	...
	while (1)
	{
		...
		Q_memset (&ent->area, 0, sizeof(ent->area));
		gi.linkentity (ent);
	}
	...
	for (i=0 ; i<globals.num_edicts ; i++)
	{
		...
		if (ent->classname)
			...
			if (Q_strcmpz(ent->classname, "target_crosslevel_target") == 0)
