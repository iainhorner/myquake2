...
//Nick - 07/09/2005 - New rando map stuff - Musashi
...
cvar_t	*deathmatch;
//cvar_t	*coop; Nick 18/09/2005 - not in DM
cvar_t	*dmflags;
...
qboolean ClientsConnected(void);
...
/* Nick - 08/09/2005 - this is gone - replaced by Musashi stuff
=================
EndDMLevel
...
void CheckDMRules (void)
{
	...
	if (level.time >= level.emp_start_time + emp_time->value) {
		if(level.isEmp)
	...
	if (level.isEmp && level.time == level.emp_start_time + (emp_time->value - 16))//  cd2emp.wav takes 16 seconds to run
...
void G_RunFrame (void)
{
	...
	// Nick - remove AI_SetSightClient ();

	// exit intermissions
	//TODO: Add scoreboardtime
	...
	{
		if (ClientsConnected() || mapcycle_always->value )
