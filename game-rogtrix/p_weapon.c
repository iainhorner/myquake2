		...
		if (! (ent->spawnflags & DROPPED_PLAYER_ITEM) )
		{
			//if (deathmatch->value)
				...
				if ((int)(dmflags->value) & DF_WEAPONS_STAY)
					...
					{
					SetRespawn (ent, 5); // Respawn team chain weapons
					}				
				else
					SetRespawn (ent, 30);
...
void Use_Weapon2 (edict_t *ent, gitem_t *item)
{
	...
	if (Q_strcmpz (item->pickup_name, "HyperBlaster") == 0)
	{
		if (item == ent->client->pers.weapon)
		{
	...
	else if (Q_strcmpz (item->pickup_name, "Railgun") == 0)
  	{
		ammo_item = FindItem(item->ammo);
		ammo_index = ITEM_INDEX(ammo_item);
		if (!ent->client->pers.inventory[ammo_index])
...
void Weapon_Generic (edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent))
{
	...
	if ((ent->client->newweapon) && (ent->client->weaponstate != WEAPON_FIRING))
	{
		...
		// Nick - quick weapon activation when chosen.
...
void Weapon_GrenadeLauncher (edict_t *ent)
{
	...
	// RAFAEL
	// Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
	// as this produces frame errors on certain weapon models when out of ammo.
	if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Grenade Launcher") == 0))
		Weapon_Generic (ent, 5, 16, 59, 64, pause_frames, fire_frames, weapon_grenadelauncher_fire);
...
void Weapon_RocketLauncher (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Rocket Launcher") == 0))
		Weapon_Generic (ent, 4, 12, 50, 54, pause_frames, fire_frames, Weapon_RocketLauncher_Fire);
...
void Weapon_Blaster (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Blaster") == 0))
		Weapon_Generic (ent, 4, 8, 52, 55, pause_frames, fire_frames, Weapon_Blaster_Fire);
...
void Weapon_HyperBlaster (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "HyperBlaster") == 0))
		Weapon_Generic (ent, 5, 20, 49, 53, pause_frames, fire_frames, Weapon_HyperBlaster_Fire);
...
void Weapon_Machinegun (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Machinegun") == 0))
		Weapon_Generic (ent, 3, 5, 45, 49, pause_frames, fire_frames, Machinegun_Fire);
...
void Weapon_Chaingun (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Chaingun") == 0))
		Weapon_Generic (ent, 4, 31, 61, 64, pause_frames, fire_frames, Chaingun_Fire);
...
void Weapon_Shotgun (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Shotgun") == 0))
		Weapon_Generic (ent, 7, 18, 36, 39, pause_frames, fire_frames, weapon_shotgun_fire);
}
...
void Weapon_SuperShotgun (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Super Shotgun") == 0))
		Weapon_Generic (ent, 6, 17, 57, 61, pause_frames, fire_frames, weapon_supershotgun_fire);
}
...
void Weapon_Railgun (edict_t *ent)
{
	...
	// RAFAEL
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Railgun") == 0))
		Weapon_Generic (ent, 3, 18, 56, 61, pause_frames, fire_frames, weapon_railgun_fire);
}
...
======================================================================
*/
// Nick - remove all this - 03/09/2005
...
void Weapon_Ionripper (edict_t *ent)
{
	...
	Weapon_Generic (ent, 4, 6, 36, 39, pause_frames, fire_frames, weapon_ionripper_fire);
        ...
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Ionripper") == 0))
		Weapon_Generic (ent, 4, 6, 36, 39, pause_frames, fire_frames, weapon_ionripper_fire);
}
...
void weapon_phalanx_fire (edict_t *ent)
{
	...
	if (ent->client->ps.gunframe == 8)
	{
		...
		fire_plasma (ent, start, forward, damage, 750, damage_radius, radius_damage);
...
void Weapon_Phalanx (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 20, 58, 63, pause_frames, fire_frames, weapon_phalanx_fire);
        ...
        // Nick - 01/11/2005 - check there is no weapon change in last Weapon_Generic call
        // as this produces frame errors on certain weapon models when out of ammo.
        if (is_quadfire && (Q_strcmpz (ent->client->pers.weapon->pickup_name, "Phalanx") == 0))
		Weapon_Generic (ent, 5, 20, 58, 63, pause_frames, fire_frames, weapon_phalanx_fire);
...
void weapon_trap_fire (edict_t *ent, qboolean held)
{
	...
	timer = ent->client->trap_time - level.time;
	speed = TRAP_MINSPEED + (TRAP_TIMER - timer) * ((TRAP_MAXSPEED - TRAP_MINSPEED) / TRAP_TIMER);
	// fire_grenade2 (ent, start, forward, damage, speed, timer, radius, held);
...
void Weapon_Trap (edict_t *ent)
{
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		...
		if (ent->client->ps.gunframe == 11)
		{
			if (!ent->client->trap_time)
			{
				ent->client->trap_time = level.time + TRAP_TIMER + 0.2;
				// RAFAEL 16-APR-98
			...
			// they waited too long, detonate it in their hand
			if (!ent->client->trap_blew_up && (level.time >= ent->client->trap_time))
			{
				ent->client->weapon_sound = 0;
			...
			{
				if (level.time >= ent->client->trap_time)
				{
					ent->client->ps.gunframe = 15;
		...
		if ((ent->client->ps.gunframe == 15) && (level.time < ent->client->trap_time))
			return;
...
//#define	HEATBEAM_DM_DMG			15 //original Damage
#define	HEATBEAM_DM_DMG			10
