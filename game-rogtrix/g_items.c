...
qboolean Pickup_Key (edict_t *ent, edict_t *other)
{
	if (coop->value)
	{
		...
		if (Q_strcmpz(ent->classname, "key_power_cube") == 0)
		{
...
void droptofloor (edict_t *ent)
{
	...
	if (tr.startsolid)
	{
		...
		if (Q_strcmpz (ent->classname, "foodcube") == 0)
		{
...
void PrecacheItem (gitem_t *it)
{
	...
	while (*s)
	{
		...
		if (!Q_strcmpz(data+len-3, "md2"))
			gi.modelindex (data);
		//else if (!strcmp(data+len-3, "sp2"))
		else if (!Q_strcmpz(data+len-3, "sp2"))
			gi.modelindex (data);
		//else if (!strcmp(data+len-3, "wav"))
		else if (!Q_strcmpz(data+len-3, "wav"))
			gi.soundindex (data);
		//if (!strcmp(data+len-3, "pcx"))
		if (!Q_strcmpz(data+len-3, "pcx"))
			gi.imageindex (data);
	}
}
...
/*QUAKED weapon_chaingun (.3 .3 1) (-16 -16 -16) (16 16 16) //No.10
*/
	{
		"weapon_chaingun", 
		Pickup_Weapon,
...
// RAFAEL 14-APR-98
/*QUAKED weapon_phalanx (.3 .3 1) (-16 -16 -16) (16 16 16)  //item 20
*/
...
/*QUAKED ammo_bullets (.3 .3 1) (-16 -16 -16) (16 16 16)//No.20
*/
	{
		"ammo_bullets",
		Pickup_Ammo,
...
/*QUAKED item_adrenaline (.3 .3 1) (-16 -16 -16) (16 16 16)//No.30
...
/*QUAKED item_bandolier (.3 .3 1) (-16 -16 -16) (16 16 16) //iem 40
*/
	{
		"item_bandolier",
		Pickup_Bandolier,
...
void InitItems (void)
{
	...
	int indexEndMarker = game.num_items-7;
		...
		if (allow_invulnerability->value > 0) {
			...
			itemlist[indexEndMarker] = invul;
			indexEndMarker++;
		}

		allow_powershield = gi.cvar("allow_powershield", "0", CVAR_LATCH);
		if (allow_powershield->value > 0) {
		    gitem_t pwrshield = {
		    "item_power_shield",
		        Pickup_PowerArmor,
		        Use_PowerArmor,
		    ...
		    itemlist[indexEndMarker] = pwrshield;
			indexEndMarker++;
		 }

		allow_bfg = gi.cvar("allow_bfg", "0", CVAR_LATCH);
		if (allow_bfg->value > 0) {
...
*/
			...
			itemlist[indexEndMarker] = bfg;
			indexEndMarker++;
		}

		/*QUAKED item_quad (.3 .3 1) (-16 -16 -16) (16 16 16)*/
		allow_quad = gi.cvar("allow_quad", "0", CVAR_LATCH);
		if (allow_quad->value > 0) {
			gitem_t quad = {
				"item_quad",
					Pickup_Powerup,
					Use_Quad,
			...
			itemlist[indexEndMarker] = quad;
			indexEndMarker++;
		}

		/*QUAKED item_power_screen (.3 .3 1) (-16 -16 -16) (16 16 16)
*/
		allow_power_screen = gi.cvar("allow_powerscreen", "0", CVAR_LATCH);
		if (allow_power_screen->value > 0) {
			gitem_t power_screen = {
					"item_power_screen", 
					Pickup_PowerArmor,
					Use_PowerArmor,
					Drop_PowerArmor,
