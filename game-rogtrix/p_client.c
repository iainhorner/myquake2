...
//	{
		...
		if (attacker && attacker->client)
		{
			...
			case MOD_TELEFRAG:
				...
				break;
			case MOD_TRAP_SPLASH:
				message = "saw";
				message2 ="'s trap explode";
				break;
			case MOD_HELD_TRAP:
				message = "stupidly watched";
				message2 = "'s trap blow up";
				break;
			case MOD_TRAP_EXPLODE:
...
//		return;
	...
	if (item && (Q_strcmpz (item->pickup_name, "Blaster") == 0))
		item = NULL;

// Nick - to make sure an 'unactivated' trap is dropped - otherwise it is thrown in think_weapon
	//if ((item && (strcmp (item->pickup_name, "Trap") == 0)) &&
	if ((item && (Q_strcmpz (item->pickup_name, "Trap") == 0)) &&
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag)
	{
		...
		self->client->respawn_time = (level.time) + 1;
		self->deadtime = level.time; // Nick - head roll stuff
	...
	{       // gib
		...
		ThrowClientHead(self, damage); // Blow player's viewpoint backwards 48 units
		self->takedamage = DAMAGE_NO; // Player cannot take anymore damage
        }
	else
	{	// normal death
		if (!self->deadflag)
...
void InitClientResp (gclient_t *client)
{
	...
	Q_memset (&client->resp, 0, sizeof(client->resp));
	client->resp.enterframe = level.framenum;
	//client->resp.coop_respawn = client->pers; Nick - 18/09/2005 - Not in DM
}
...
float	PlayersRangeFromSpot (edict_t *spot)
{
	...
	edict_t         *target = NULL; // Nick - check for traps on spawn
	//int 		match; //debug	
	...
	for (n = 1; n <= maxclients->value; n++)
	{
		...
		playerdistance = VectorLength (v);
                ...
                if (playerdistance == 10.0 || playerdistance == 8.125) // Nick - This spot has a player
                // (VectorLength is 10.00 only if countdown is on (normally 8.125)).
                        playerdistance = rand() % 100;  // Force a random playerdistance value < 100

		if (playerdistance < bestplayerdistance)
			bestplayerdistance = playerdistance;
	}

        // Nick detect (if) trap radius from spawn point
        while ( ((target = findradius(target, spot->s.origin, 512)) != NULL) )
        {
        //if (target != NULL)
        //gi.bprintf (PRINT_HIGH, "Classname:%s\n", target->classname); // debug - overflows :(

	//Trap found nearby, so psuedo-make it the closest by returning spot dead centre.
	//if ( target->classname && (strcmp(target->classname, "htrap") == 0) )
	//match = (Q_strcmpz(target->classname, "htrap"));
        //gi.bprintf (PRINT_HIGH, "Match:%d\n", match);

	if ( target->classname && (Q_strcmpz(target->classname, "htrap") == 0) )
                {
                //VectorSubtract (spot->s.origin, target->s.origin, v);
                //playerdistance = VectorLength (v);

                //if (playerdistance < bestplayerdistance)
                        //bestplayerdistance = playerdistance;
                        bestplayerdistance = 0.0; // Nick - make the trap the closest!
                }
        }
        // End Nick
...
edict_t *SelectFarthestDeathmatchSpawnPoint (void)
{
	...
	// Nick - make the first in spawn randomly when all spawns are empty at map change
			if (level.time < 15)  // map less than 15 seconds old
			{
				if (bestplayerdistance > 9999991.0) // no players in yet.
				bestplayerdistance = (rand() % 600) + 400.0; // force a random spawn point here.
                	}
...
void body_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (self->health < -40)
	{
		gi.sound (self, CHAN_BODY, gi.soundindex (GIB_SOUND), 1, ATTN_NORM, 0);
			for (n= 0; n < 4; n++)
...
void spectator_respawn (edict_t *ent)
{
	...
	//TODO: send commands to client here
	if (ent->client->pers.spectator) {
		char *value = Info_ValueForKey (ent->client->pers.userinfo, "spectator");
		if (*spectator_password->string && 
			//strcmp(spectator_password->string, "none") && 
			//strcmp(spectator_password->string, value)) {
			Q_strcmpz(spectator_password->string, "none") && 
			Q_strcmpz(spectator_password->string, value)) {
		...
		if (*password->string && Q_strcmpz(password->string, "none") && 
			Q_strcmpz(password->string, value)) {
	...
	// Nick 29/08/2005 - and clear deaths ;-)
	ent->client->pers.deaths = ent->client->resp.deaths = 0;
	...
	if (ent->client->pers.spectator) 
		gi.bprintf (PRINT_HIGH, "%s has moved to the girls room\n", ent->client->pers.netname);
	else
...
void PutClientInServer (edict_t *ent)
{
	...
	//{
		...
		Q_memcpy (userinfo, client->pers.userinfo, sizeof(userinfo));
...
//              }
	...
	ent->mass = 200;
	// Nick - New player joining midway thru' map gets respawn value set from NULL.
	if (!ent->client->respawn_time) //&& (level.time > countdelay->value) )
		ent->client->respawn_time = (level.time);
...
// Nick
	...
	Q_memset (&ent->client->ps, 0, sizeof(client->ps));
	...
	if (client->pers.spectator) {
		...
		// Nick 17/09/2005 - it stays SOLID_NOT on re-entering game!
		ent->solid = SOLID_BBOX;
...
void ClientBeginDeathmatch (edict_t *ent)
{
	...
	gi.cprintf(ent, PRINT_CHAT, "\nWelcome %s\n", ent->client->pers.netname);
	gi.cprintf(ent, PRINT_CHAT, "Join us on Discord, details @ DieHardGames.UK\n");
	gi.cprintf(ent, PRINT_CHAT, "\n*********************************************\n");

	gi.bprintf (PRINT_HIGH, "%s entered the game\n", ent->client->pers.netname);

	//TODO: play welcome sound?
	//gi.sound(ent, CHAN_AUTO, gi.soundindex("ut_sounds/cd5min.wav"), 1, ATTN_NORM, 0);
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	if (*value && Q_strcmpz(value, "0")) {
		...
		if (*password->string && Q_strcmpz(password->string, "none") && 
			...
			// Nick - next 3 lines to stop passworded crash
			ent->solid = SOLID_NOT;
			ent->inuse = false;
			G_FreeEdict (ent);
...
// Nick - 20/04/2003 - Start new level spawn move delay code.
	if ( ((level.time) < countdelay->value) 
	     && (!ent->client->pers.spectator) )
	{
		if (ent->solid != SOLID_NOT)  // Here make body 'ghost' in case two
		ent->solid = SOLID_NOT;       // or more clients spawn on same spot.

                if (((countdelay->value) - 3) == (level.time)) {
                gi.sound(ent, CHAN_VOICE, gi.soundindex("world/3.wav"), 1, ATTN_NORM, 0);
                return;
		}
                else if (((countdelay->value) - 2) == (level.time)) {
		gi.sound(ent, CHAN_VOICE, gi.soundindex("world/2.wav"), 1, ATTN_NORM, 0);
		return;
		}
                else if (((countdelay->value) - 1) == (level.time)) {
                gi.sound(ent, CHAN_VOICE, gi.soundindex("world/1.wav"), 1, ATTN_NORM, 0);
		return;
		}
		return;
	}
		if ( (countdelay->value == (level.time))
		&& (!ent->client->pers.spectator) )
                gi.sound(ent, CHAN_VOICE, gi.soundindex("world/brkglas.wav"), 1, ATTN_NORM, 0);

// Nick - hack to check if clear of bodies

	if ( (countdelay->value) && (ent->solid == SOLID_NOT)
		&& (!ent->client->pers.spectator) && (countdelay->value <= level.time) 
		&& (ent->client->respawn_time <= countdelay->value + 1) )
	{

	edict_t	*player;
	int		n;
	vec3_t diff;
	qboolean freebod = true;
		...
		for (n = 1; n <= maxclients->value; n++)
		{
			...
			VectorSubtract (ent->s.origin, player->s.origin, diff);
                	...
                	if ( (diff[0] >= -36 && diff[0] <= 36) 
		   	&& (diff[1] >= -36 && diff[1] <= 36) 
		   	&& (diff[2] >= -66 && diff[2] <= 66) )
                        {
		               freebod = false;
                                break; // if it is false (players overlapping), no need to check more
			...
			if (freebod)
				ent->solid = SOLID_BBOX; //Remove 'ghost'.
//gi.bprintf (PRINT_HIGH, "bodyfree leveltime: %f bodyfree spawntime: %f\n", level.time, ent->client->respawn_time);

	}


// Nick 20/04/2003 - End new level spawn move delay code.
	...
	if (level.intermissiontime)
	{
		...
		if (level.time > level.intermissiontime + 7.0 // Nick- Knock up to 7 secs so I got time to read the scores!!! 
	...
	if (ent->client->chase_target) {
		...
		if (Q_memcmp(&client->old_pmove, &pm.s, sizeof(pm.s)))
		{
			pm.snapinitial = true;
	//		gi.dprintf ("pmove changed!\n");
		...
		if (ent->deadflag)
		{
			if (level.time < (ent->deadtime + 1.5) ) {
                        	client->ps.viewangles[ROLL] = 40*sin(level.time);
			}
...
void ClientBeginServerFrame (edict_t *ent)
{
	...
	if (ent->deadflag)
	{
		...
		if ( level.time > client->respawn_time)
		{
			...
			//if (deathmatch->value)
			if (level.time > (ent->deadtime + 0.3) ) // wait 1/3 seconds to stop accidental
				buttonMask = BUTTON_ATTACK;
			else
				client->latched_buttons = 0; // This will stop accidental respawn
			//	buttonMask = -1;
