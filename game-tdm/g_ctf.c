...
char *CTFTeamName(int team)
{
	switch (team) {
	case CTF_TEAM1:
		return team_a_name->string;
	case CTF_TEAM2:
		return team_b_name->string;
	}
...
void CTFAssignSkin(edict_t *ent, char *s)
{
	...
	char *p;
	char t[64];
	char* model_a_with_bs;
	char* model_b_with_bs;
	const char* bs = "/";
	...
	case CTF_TEAM1:	
		model_a_with_bs = malloc(strlen(team_a_model->string) + 1 + 4); /* make space for the new string (should check the return value ...) */
		strcpy(model_a_with_bs, team_a_model->string); /* copy name into the new var */
		strcat(model_a_with_bs, bs); /* add the extension */
		strcpy(t, model_a_with_bs);
		gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\%s%s", ent->client->pers.netname, t, team_a_skin->string) );
		break;
	case CTF_TEAM2:
		model_b_with_bs = malloc(strlen(team_b_model->string) + 1 + 4); /* make space for the new string (should check the return value ...) */
		strcpy(model_b_with_bs, team_b_model->string); /* copy name into the new var */
		strcat(model_b_with_bs, bs); /* add the extension */
		strcpy(t, model_b_with_bs);
		gi.configstring (CS_PLAYERSKINS+playernum,va("%s\\%s%s", ent->client->pers.netname, t, team_b_skin->string) );
		break;
	default:
...
void CTFScoreboardMessage (edict_t *ent, edict_t *killer)
{
	...
	*string = 0;
	...
	sprintf(string, "if 24 xv 8 yv 8 pic 24 endif "
		"xv 50 yv 28 string \"%s\" "
		"xv 98 yv 12 num 2 18 "
		"if 25 xv 168 yv 8 pic 25 endif "
		"xv 210 yv 28 string \"%s\" "
		"xv 256 yv 12 num 2 20 ",
		team_a_name->string,
		team_b_name->string,
		totalscore[0], total[0],
...
pmenu_t joinmenu[] = {
	{ "*Quake II",			PMENU_ALIGN_CENTER, NULL },
	{ "*Team Deathmatch",	PMENU_ALIGN_CENTER, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL },
...
void CTFJoinTeam(edict_t *ent, int desired_team)
{
	...
	if (ent->client->resp.ctf_team == desired_team) {
		gi.cprintf(ent,PRINT_HIGH, "you are already in the %s team.\n", CTFTeamName(desired_team));
		return;
	}
...
char* fulljoinAstr = malloc(strlen(joinstr) + strlen(team_a_name->string) + strlen(teamstr) + 1);
strcpy(fulljoinAstr, joinstr);
strcat(fulljoinAstr, team_a_name->string);
strcat(fulljoinAstr, teamstr);

char* fulljoinBstr = malloc(strlen(joinstr) + strlen(team_b_name->string) + strlen(teamstr) + 1);
strcpy(fulljoinBstr, joinstr);
strcat(fulljoinBstr, team_b_name->string);
strcat(fulljoinBstr, teamstr);
	...
	if (ctfgame.match >= MATCH_PREGAME && matchlock->value) {
		...
		if (ctfgame.match >= MATCH_PREGAME) {
			joinmenu[jmenu_red].text = fulljoinAstr;
			joinmenu[jmenu_blue].text = fulljoinBstr;
		} else {
			joinmenu[jmenu_red].text = fulljoinAstr;
			joinmenu[jmenu_blue].text = fulljoinBstr;
		}
