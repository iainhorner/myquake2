...
void AngleMove_Begin (edict_t *ent)
{
	...
	VectorScale (destdelta, 1 / traveltime, ent->avelocity);
...
void plat_Accelerate (moveinfo_t *moveinfo)
{
	...
	if (moveinfo->current_speed == moveinfo->move_speed)
		if ((moveinfo->remaining_distance - moveinfo->current_speed) < moveinfo->decel_distance)
		{
			...
			p2_distance = moveinfo->move_speed * (1 - (p1_distance / moveinfo->move_speed));
	...
	if (moveinfo->current_speed < moveinfo->speed)
	{
		...
		// entire move
		p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
		p1_speed = (old_speed + moveinfo->move_speed) / 2;
		p2_distance = moveinfo->move_speed * (1 - (p1_distance / p1_speed));
		distance = p1_distance + p2_distance;
...
void door_go_down (edict_t *self)
{
	...
	if (Q_strcmpz(self->classname, "func_door") == 0)
		Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
	//else if (strcmp(self->classname, "func_door_rotating") == 0)
	else if (Q_strcmpz(self->classname, "func_door_rotating") == 0)
...
void trigger_elevator_init (edict_t *self)
{
	...
	if (Q_strcmpz(self->movetarget->classname, "func_train") != 0)
	{
...
/* Nick - 18/09/2005 - remove all this
