...
void target_lightramp_use (edict_t *self, edict_t *other, edict_t *activator)
{
	if (!self->enemy)
	{
		...
		while (1)
		{
			...
			if (Q_strcmpz(e->classname, "light") != 0)
			{
...
void SP_target_lightramp (edict_t *self)
{
	...
	if (!self->message || Q_strlenz(self->message) != 2 || self->message[0] < 'a' || self->message[0] > 'z' || self->message[1] < 'a' || self->message[1] > 'z' || self->message[0] == self->message[1])
	{
